...
const STATUS_FILTERS = ['', 'submitted', 'admin_review', 'approved', 'rejected', 'active', 'suspended', 'draft'];
...
export function CarriersListPage() {
  ...
  const [status, setStatus] = useState(() => {
    ...
    return STATUS_FILTERS.includes(q) ? q : '';
  });
  ...
  const load = useCallback(async (p: number, s: string) => {
    ...
    try { setData(await carrierApi.list(p, 10, s || undefined)); }
