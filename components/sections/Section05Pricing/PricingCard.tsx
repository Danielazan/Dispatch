...
export default function PricingCard({ plan, index }: { plan: PricingPlan; index: number }) {
  return (
    ...
    >
      {/* brass label strip — drawn by entrance; carries the specular catch-light */}
      {plan.featured && (
        <div data-featured-strip className="absolute inset-x-0 top-0 origin-left overflow-hidden bg-[var(--s5-brass)]">
          <span
            data-s5="catchlight"
            aria-hidden="true"
            className="pointer-events-none absolute inset-y-0 left-0 w-2/5 opacity-0"
            style={{ background: "linear-gradient(100deg, transparent 0%, rgba(240,236,228,0.85) 50%, transparent 100%)" }}
          />
          <span
            data-featured-badge
            className="relative flex h-7 items-center justify-center text-[10px] font-semibold tracking-[0.22em] text-[#111820]"
          >
            {plan.badge}
          </span>
        </div>
