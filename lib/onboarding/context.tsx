...
export interface SessionData {
  ...
  furthestCompletedSection: number;
  lead: LeadData | null;
...
function reducer(state: OnboardingState, action: Action): OnboardingState {
  switch (action.type) {
    ...
    case 'SET_SESSION':
      return { ...state, sessionData: action.data, status: isLocked(action.data) ? 'submitted' : 'ready' };
    case 'SET_STEP':
      return { ...state, currentStepIndex: Math.max(0, Math.min(6, action.index)) };
    case 'UPDATE_FIELD': {
      ...
      return {
        ...
        sessionData: {
          ...state.sessionData,
          carrier: { ...(state.sessionData.carrier ?? {}), [action.key]: action.value } as CarrierData,
        },
      };
    }
    case 'SET_SAVE_STATE':
      return { ...state, saveState: action.state, lastSavedAt: action.timestamp ?? state.lastSavedAt };
    case 'SET_ERROR':
...
export function OnboardingProvider({ token, children }: { token: string; children: ReactNode }) {
    ...
    try {
      ...
      const e =
        err instanceof OnboardingApiError
          ? err
          : new OnboardingApiError(0, 'network_error', 'Connection problem — check your network.');
  ...
  const updateField = useCallback((key: keyof CarrierData, value: string | null) => {
    dispatch({ type: 'UPDATE_FIELD', key, value });
  }, []);

  const saveSection = useCallback(
    async (section: number, data: Record<string, unknown>): Promise<boolean> => {
      dispatch({ type: 'SET_SAVE_STATE', state: 'saving' });
      dispatch({ type: 'SET_ERROR', error: null });
      try {
        const res = await obApi.saveSection(token, section, data);
        dispatch({
          type: 'SET_SESSION',
          data: {
            ...(stateRef.current.sessionData ?? ({} as SessionData)),
            carrier: res.carrier,
            furthestCompletedSection: res.furthestCompletedSection,
          },
        });
  ...
  const refreshSession = useCallback(async () => {
    try {
      ...
      /* silent — keep current UI on transient failure */
    }
  }, [token]);

  const registerSceneSave = useCallback((fn: (() => Promise<boolean>) | null) => {
    sceneSaveRef.current = fn;
  }, []);

  const triggerSave = useCallback(async (): Promise<boolean> => {
    if (!sceneSaveRef.current) return false;
    return sceneSaveRef.current();
  }, []);

  const saveAndContinue = useCallback(async () => {
    const ok = await triggerSave();
    if (ok) dispatch({ type: 'SET_STEP', index: stateRef.current.currentStepIndex + 1 });
  }, [triggerSave]);

  return (
    <OnboardingContext.Provider
      value={{ state, goToStep, updateField, saveSection, registerSceneSave, triggerSave, saveAndContinue, reload: load, refreshSession }}
    >
      {children}
    </OnboardingContext.Provider>
  );
...
export function useSceneSave(fn: () => Promise<boolean>) {
  ...
  fnRef.current = fn;
  useEffect(() => {
    registerSceneSave(() => fnRef.current());
    return () => registerSceneSave(null);
  }, [registerSceneSave]);
}